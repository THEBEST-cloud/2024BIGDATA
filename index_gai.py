...
def invoke(inputs):
    ...
    for i in range(2):
        if i == 0:
            data = np.load(os.path.join(test_data_root_path, "temp.npy")) # (N, L, S, 1)
        else: 
            data = np.load(os.path.join(test_data_root_path, "wind.npy"))
        N, L, S = data.shape # 72, 168, 60
        ...
        if i == 0:
            model.load_state_dict(torch.load(r"checkpoints\v1_iTransformer_Meteorology_ftMS_sl168_ll1_pl24_dm64_nh1_el1_df64_global_temp\checkpoint_0.pth"))
        else:
            model.load_state_dict(torch.load(r"checkpoints\v1_iTransformer_Meteorology_ftMS_sl168_ll1_pl24_dm64_nh1_el1_df64_global_wind\checkpoint_0.pth"))
        outputs = model(data)
...
if __name__ == "__main__":
    inputs = r"D:\work\大数据挑战赛\global\global"
    invoke(inputs)